...
\zirconia\ is unusual in exhibiting three commonly reported polytypes in its binary phase diagram (see Figure \ref{figure:binary_phase_diagram}). The temperature-pressure phase diagram of \zirconia\ as is currently understood is shown in Figure \ref{figure:currentphasediagram}, with details of these phases collated in Table \ref{table:phases}. Each will now be described and contrasted.

\begin{figure}[htp] % Current phase diagram
\centering
\includegraphics[height=8.5cm]{images/current_phase_diagram.png}
...
\begin{tikzpicture}
	\begin{axis}
        ...
        \addplot[no marks, densely dotted] table [x=zr_o_dist, y=cubic,]{dat/zr_o_bond_distances.dat}; \addlegendentry{Cubic};
			\end{axis}
		\end{tikzpicture}
		\caption{Density plot of the nearest neighbour Zr-O bond distances in \zirconia\ for each crystal structure. Specific volumes are 11.99 \r{A}$^{3}$ion$^{-1}$, 11.51 \r{A}$^{3}$ion$^{-1}$, and 11.13 \r{A}$^{3}$ion$^{-1}$ for monoclinic, tetragonal, and cubic phases respectively.}
...
A hexagonal phase of \zirconia\ has also been reported in X-ray diffraction studies at high pressure (20 GPa) and above 1000 \textdegree C \cite{ohtaka1994new}. This phase retains its structure during isobaric quenching, but turns back to monoclinic once pressure is released. This phase is only observed under extreme conditions, and so is mentioned here for purposes of completion.
